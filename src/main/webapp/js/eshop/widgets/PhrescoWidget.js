    ...
    Y.extend(PhrescoWidget, Y.Widget, {
       ...
       _isEmpty : function(str) {
        ...
        showProductsFromCache : function() {
            ...
            for (var i = 0; i < listeners.length; i++) {
                ...
                var target = listeners[i].get("targetNode");
                widgetObj.loading(target);
                //apiRef.populateResponseToWidgets(widgetObj.get("widgets"), data, listeners[i]);
                //widgetObj.get("widgets")
                if (data != "") {
       ...
       addTestListener : function(widget) {
        ...
        removeProduct : function() {
			...
			//console.info('removedItem = ' , removedItem);
            var totalItem = 0; 
            var cartTotal = 0;
            if (productArray.length != 0) {
        ...
        validateFormFields : function (validateFormFields, apiRef) {
         ...
         showProductDetails : function() {
            ...
            for (var i = 0; i < listeners.length; i++) {
                ...
                //apiRef.getProductReviews(widgetObj.get("widgets"), this.data, listeners[i]);
            }
        },
        ...
        userLogin: function () {
            ...
            /*if(sessionStorage.userId){
                console.info('sessionStorage.userId',sessionStorage.userId);
            }*/ 
        ...
        /*registerTabWidget:function(){
           ...
           var listeners = widgetObj.get("onSelectedMenuListener");
            for (var i = 0; i < listeners.length; i++) {
                ...
                if (listeners[i] instanceof Y.Phresco.RegisterWidget) {
                    var target = listeners[i].get("targetNode");
                    widgetObj.loading(target);
                    listeners[i].onUpdateListener(null);
                }
